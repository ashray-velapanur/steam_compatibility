...
def create(profile, user):
    ...
    friend_user = User.get_or_insert(key_name=id, games=recently_played_games, name=name, avatar=avatar, genre_scores=json.dumps(friend_genre_scores))
    Friend(key_name=id, parent=user, user=friend_user, compatibility_score=compatibility_score).put()
